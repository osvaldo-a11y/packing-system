...
AFTER = Path("/opt/cursor/artifacts/agregar-material-desktop-ux-final")
PREV = Path("/opt/cursor/artifacts/agregar-material-desktop/after/B_implementado_vacio.png")
OUT = Path("/opt/cursor/artifacts/agregar-material-desktop-ux-final/gates")
OUT.mkdir(parents=True, exist_ok=True)

PAIRS = [
    ("B_material_rapido_desktop.png", "material_rapido_desktop.png", "Material rápido Desktop"),
    ("C_material_rapido_mobile.png", "material_rapido_mobile.png", "Material rápido Mobile"),
...
if PREV.exists() and (AFTER / "A_vacio.png").exists():
    a = Image.open(PREV).convert("RGB")
    b = Image.open(AFTER / "A_vacio.png").convert("RGB")
    w, h = max(a.size[0], b.size[0]), max(a.size[1], b.size[1])
    side = Image.new("RGB", (w * 2 + 24, h), (17, 17, 17))
    side.paste(a, (0, 0))
    side.paste(b, (w + 24, 0))
    side.save(AFTER / "E_side_by_side.png")
    print("side-by-side OK")
